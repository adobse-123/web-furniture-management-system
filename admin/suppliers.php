...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['action']) && $_POST['action'] === 'create') {
    create_supplier($_POST);
    header('Location: suppliers.php'); exit;
}

if (isset($_GET['delete'])) { delete_supplier($_GET['delete']); header('Location: suppliers.php'); exit; }
?>
<!doctype html>
<html><head><meta charset="utf-8"><title>Suppliers</title>
...
<div class="row">
    <div class="col-md-6">
        ...
        <form method="post">
            ...
            <div class="mb-2"><label>Contact Person</label><input name="contact_person" class="form-control"></div>
        ...
        <table class="table">
            ...
            <?php foreach($suppliers as $s): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($s['contact_person']) ?></td>
                    <td><?= htmlspecialchars($s['phone']) ?></td>
                    <td><a class="btn btn-sm btn-danger" href="suppliers.php?delete=<?= $s['id'] ?>">Delete</a></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
