...
function create_material($data)
{
    ...
    return $stmt->execute([
        $data['name'], $data['description'], $data['unit'], $data['current_quantity'], $data['alert_threshold'], $data['unit_price']
    ]);
}
...
function update_material($id, $data)
{
    ...
    return $stmt->execute([
        $data['name'], $data['description'], $data['unit'], $data['current_quantity'], $data['alert_threshold'], $data['unit_price'], $id
    ]);
}
...
{
    // $data: material_id, type (IN/OUT/ADJUST), quantity, price_per_unit, supplier_id|null, order_id|null, notes
    $pdo = get_pdo();
    try {
        $pdo->beginTransaction();

        $stmt = $pdo->prepare("INSERT INTO stock_transactions (raw_material_id, type, quantity, price_per_unit, supplier_id, order_id, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt->execute([
            $data['material_id'], $data['type'], $data['quantity'], $data['price_per_unit'] ?? null, $data['supplier_id'] ?? null, $data['order_id'] ?? null, $data['notes'] ?? null
        ]);
        ...
        if ($data['type'] === 'IN') {
            ...
            // ADJUST expects 'new_quantity' or 'quantity' used as delta
